...
#ifndef __ESPUINO_SETTINGS_LOLIN32S3_H__
#define __ESPUINO_SETTINGS_LOLIN32S3_H__
    #include "Arduino.h"
    ...
    */
    ...
    // Further infos: https://randomnerdtutorials.com/esp32-pinout-reference-gpios/
        ...
        #define SPISD_CS                    46          // GPIO for chip select (SD)
            ...
            #define SPISD_MOSI              11          // GPIO for master out slave in (SD) => not necessary for single-SPI
            #define SPISD_MISO              13          // GPIO for master in slave ou (SD) => not necessary for single-SPI
            #define SPISD_SCK               12          // GPIO for clock-signal (SD) => not necessary for single-SPI
        #endif
    #endif
    ...
    #define POWER                           48          // GPIO used to drive transistor-circuit, that switches off peripheral devices while ESP32-deepsleep
    #ifdef POWER
    ...
    // (optional) Neopixel
    #define LED_PIN                          7          // GPIO for Neopixel-signaling

    // (optinal) Headphone-detection
    #ifdef HEADPHONE_ADJUST_ENABLE
        //#define DETECT_HP_ON_HIGH                       // Per default headphones are supposed to be connected if HT_DETECT is LOW. DETECT_HP_ON_HIGH will change this behaviour to HIGH.
        #define HP_DETECT                    47         // GPIO that detects, if there's a plug in the headphone jack or not
    #endif

    // (optional) Monitoring of battery-voltage via ADC
    #ifdef MEASURE_BATTERY_VOLTAGE
        #define VOLTAGE_READ_PIN            3          // GPIO used to monitor battery-voltage. Change to 35 if you're using Lolin D32 or Lolin D32 pro as it's hard-wired there!
